...
struct Inner {
    pub value: Vec<isize>,
    munged: Option<Vec<isize>>,
}

impl Inner {
    pub fn new(value: Vec<isize>) -> Inner {
        Inner {
            value: value,
            munged: None,
        }
    ...
    #[allow(dead_code)]
    pub fn set(&mut self, value: Vec<isize>) {
        self.value = value;
        self.munged = None;
    }
    ...
    pub fn munged(&self) -> Cow<Vec<isize>, [isize]> {
        match self.munged {
...
mucell_ref_type! {
    ...
    impl Deref -> [isize],
    data: Cow<'a, Vec<isize>, [isize]> = |x| x.munged()
}
